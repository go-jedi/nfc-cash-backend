...
func (r *TelegramPostgres) BotCreate(botForm appl_row.BotCreate) (bool, int, error) {
	...
	err = r.db.QueryRow("SELECT bot_create($1)", botFormJson).Scan(&isBotCreate)
	if err != nil {
...
func (r *TelegramPostgres) BotDelete(botForm appl_row.BotDelete) (bool, int, error) {
	var isBotDelete bool
	botFormJson, err := json.Marshal(botForm)
	if err != nil {
		return false, http.StatusInternalServerError, fmt.Errorf("ошибка конвертации botForm, %s", err)
	}
	err = r.db.QueryRow("SELECT bot_delete($1)", botFormJson).Scan(&isBotDelete)
	if err != nil {
...
func (r *TelegramPostgres) GetBots(uid string) ([]appl_row.Bot, int, error) {
	var bots []appl_row.Bot
	var botsByte []byte
	err := r.db.QueryRow("SELECT bots_get($1)", uid).Scan(&botsByte)
	if err != nil {
